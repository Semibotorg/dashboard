    ...
    data?.youtubeChannelName.forEach(async (username) => {
      ...
      await youtube.channels.list(
        ...
        async (err, res) => {
          ...
          const NOW = Date.now();
          try {
            const resVideos = await parser.parseURL(
            ...
            const youtubeLogo =
              "https://img.icons8.com/color/48/null/youtube-play.png";
            const videoThumbnail = `http://i1.ytimg.com/vi/${videoId}/hqdefault.jpg`;
            if (!dataHistory) return;
            const historyFilter = dataHistory?.YoutubeHistory.filter(
              (el) => el == videoId
            );

            if (historyFilter.length > 0) return;
            const channelDiscord: any = await client.channels.fetch(
              data.channelId
            );
